...
// 2~20 영문, 한글, 숫자 입력 가능. 특수문자는 !@#())*_-로 한정. 띄어쓰기 가능.
...
export const userAccountValidation = Yup.object({
  ...
  lunchQty: Yup.number()
    .nullable()
    .typeError('숫자만 입력하세요.')
    .integer('0이상의 숫자를 입력해 주세요.')
    .min(0, '0이상의 숫자를 입력해 주세요.'),
...
export const bankAccountValidation = Yup.object({
  ...
  bankName: Yup.string('')
    ...
    .required('은행명을 입력하세요.'),
  accountNo: Yup.string('').required('' - '를 포함해서 계좌번호를 입력하세요.'),
});
...
export const employeeValidation = Yup.object({
  ...
  accountNo: Yup.string('' - '를 포함해서 계좌번호를 입력하세요.'),
});
